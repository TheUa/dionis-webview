...
import static the.ua.dionis_view.AgentWebUtils.getAgentWebFilePath;
...
public class AgentWebConfig {
    ...
    private static final String TAG = the.ua.dionis_view.AgentWebConfig.class.getSimpleName();
    ...
    public static synchronized void clearDiskCache(Context context) {
        try {
            the.ua.dionis_view.AgentWebUtils.clearCacheFolder(new File(getCachePath(context)), 0);
            String path = getExternalCachePath(context);
            if (!TextUtils.isEmpty(path)) {
                File mFile = new File(path);
                the.ua.dionis_view.AgentWebUtils.clearCacheFolder(mFile, 0);
            }
