...
public class DefaultDownloadImpl implements android.webkit.DownloadListener {
    ...
    private static final String TAG = the.ua.dionis_view.DefaultDownloadImpl.class.getSimpleName();

    protected the.ua.dionis_view.PermissionInterceptor mPermissionListener = null;
    /**
    ...
    protected DefaultDownloadImpl(Activity activity, WebView webView, the.ua.dionis_view.PermissionInterceptor permissionInterceptor) {
        this.mContext = activity.getApplicationContext();
        this.mActivityWeakReference = new WeakReference<Activity>(activity);
        this.mPermissionListener = permissionInterceptor;
        this.mAgentWebUIController = new WeakReference<AbsAgentWebUIController>(the.ua.dionis_view.AgentWebUtils.getAgentWebUIControllerByWebView(webView));
        try {
    ...
    protected List<String> checkNeedPermission() {
        ...
        if (!the.ua.dionis_view.AgentWebUtils.hasPermission(mActivityWeakReference.get(), AgentWebPermissions.STORAGE)) {
    ...
    protected void preDownload(String url) {
        if (!isForceRequest(url) &&
                the.ua.dionis_view.AgentWebUtils.checkNetworkType(mContext) > 1) {
            showDialog(url);
            return;
        }
        performDownload(url);
    ...
    public static the.ua.dionis_view.DefaultDownloadImpl create(@NonNull Activity activity,
                                                               @NonNull WebView webView,
                                                               @Nullable the.ua.dionis_view.PermissionInterceptor permissionInterceptor) {
        return new the.ua.dionis_view.DefaultDownloadImpl(activity, webView, permissionInterceptor);
    }
}
