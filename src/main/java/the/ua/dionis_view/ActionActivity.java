...
public final class ActionActivity extends Activity {
    ...
    private static final String TAG = the.ua.dionis_view.ActionActivity.class.getSimpleName();
    private Action mAction;
    public static final int REQUEST_CODE = 0x254;

    public static void start(Activity activity, Action action) {
        Intent mIntent = new Intent(activity, the.ua.dionis_view.ActionActivity.class);
...
//        mIntent.setExtrasClassLoader(Action.class.getClassLoader());
    ...
    private void permission(Action action) {
        ...
        if (the.ua.dionis_view.AgentWebUtils.isEmptyCollection(permissions)) {
            mPermissionListener = null;
            mRationaleListener = null;
            finish();
            return;
    ...
    private void realOpenCamera() {
        try {
            ...
            File mFile = the.ua.dionis_view.AgentWebUtils.createImageFile(this);
            if (mFile == null) {
                mChooserListener.onChoiceResult(REQUEST_CODE, Activity.RESULT_CANCELED, null);
                mChooserListener = null;
                finish();
            }
            Intent intent = the.ua.dionis_view.AgentWebUtils.getIntentCaptureCompat(this, mFile);
    ...
    private void realOpenVideo(){
        try {
            ...
            File mFile = the.ua.dionis_view.AgentWebUtils.createVideoFile(this);
            if (mFile == null) {
                mChooserListener.onChoiceResult(REQUEST_CODE, Activity.RESULT_CANCELED, null);
                mChooserListener = null;
                finish();
            }
            Intent intent = the.ua.dionis_view.AgentWebUtils.getIntentVideoCompat(this, mFile);
            mUri = intent.getParcelableExtra(EXTRA_OUTPUT);
            this.startActivityForResult(intent, REQUEST_CODE);
        } catch (Throwable ignore) {
            if (mChooserListener != null) {
